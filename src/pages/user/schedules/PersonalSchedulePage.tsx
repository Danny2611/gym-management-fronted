...
const PersonalSchedulePage: React.FC = () => {
  ...
  const fetchWorkoutSchedules = async () => {
    try {
      ...
      if (response.success && response.data) {
        // Ensure consistent date format before setting state
        ...
        setWorkoutSchedules(formattedWorkouts);
      } else {
        console.error(
  ...
  const combineSchedules = () => {
    ...
    workoutSchedules.forEach((workout) => {
      // Ensure proper date formatting for comparison
      const workoutDate = workout.date
        ? format(new Date(workout.date), "yyyy-MM-dd")
        : null;

      // Format time from timeStart
      let formattedTime = null;
      if (workout.timeStart) {
        const timeStartObj = new Date(workout.timeStart);
        formattedTime = {
          startTime: format(timeStartObj, "HH:mm"),
          endTime: format(
            add(timeStartObj, { minutes: workout.duration || 60 }),
            "HH:mm",
          ),
        };
      }
      ...
      combined.push({
        ...
        time: formattedTime, // Add time to match PT schedule format
    ...
    combined.sort((a, b) => {
      ...
      if (a.type === "appointment" && a.time) {
        timeA = a.time.startTime;
      } else if (a.type === "workout" && a.timeStart) {
        timeA = format(new Date(a.timeStart), "HH:mm");
      }

      if (b.type === "appointment" && b.time) {
        timeB = b.time.startTime;
      } else if (b.type === "workout" && b.timeStart) {
        timeB = format(new Date(b.timeStart), "HH:mm");
      }

      return timeA.localeCompare(timeB);
    });
    const validSchedules = combined.filter(
      (schedule) => schedule.date !== null,
    );
    setCombinedSchedules(validSchedules);
  };
  ...
  const hasSchedulesOnDate = (date: Date) => {
    ...
    return combinedSchedules.some((schedule) => {
      // Apply schedule type filter if selected
  ...
  const countPtSchedulesOnDate = (date: Date) => {
    const dateStr = format(date, "yyyy-MM-dd");
    return combinedSchedules.filter((schedule) => {
      // Only count PT schedules if viewing all or only PT schedules
  ...
  const countWorkoutSchedulesOnDate = (date: Date) => {
    const dateStr = format(date, "yyyy-MM-dd");
    return combinedSchedules.filter((schedule) => {
      // Only count workout schedules if viewing all or only workout schedules
              ...
              <div className="divide-y divide-gray-200 dark:divide-gray-700">
                {filteredSchedules.map((schedule) => (
                  <ScheduleCard
                    ...
                    schedule={schedule}
                    onMarkCompleted={() =>
                      markAsCompleted(schedule._id, schedule.type)
                    }
